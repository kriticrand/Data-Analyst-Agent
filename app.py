...
if not GEMINI_KEYS:
    logger.warning("No Gemini API keys found. Agent may not function.")
            ...
            for key in self.keys:
                try:
                    ...
                    if any(qk in msg for qk in QUOTA_KEYWORDS):
                        logger.warning(f"Quota error for key ending in ...{key[-4:]} with model {model}. Trying next.")
                    else:
                        logger.error(f"Failed to initialize LLM for model {model} with key ending in ...{key[-4:]}: {e}")
                    time.sleep(0.5)
...
def clean_llm_output(output: str) -> Dict:
    try:
        ...
        # Aggressive cleaning: strip any leading non-JSON text
        s = re.sub(r"^.*?({.*})", r"\1", output.strip(), flags=re.DOTALL)
...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    injected_vars = ""
    if injected_pickle:
        injected_vars += f"df = pd.read_pickle(r'''{injected_pickle}''')\n"
        injected_vars += "data = df.to_dict(orient='records')\n"
    else:
        injected_vars += "df = globals().get('df', pd.DataFrame())\n"
        injected_vars += "data = globals().get('data', {})\n"
...
'''
    ...
    script_lines.append(injected_vars)
    script_lines.append(helper)
    script_lines.append("\nresults = {}\n")
    script_lines.append("try:")
    script_lines.extend(["    " + line for line in code.splitlines()])
    script_lines.append("\nexcept Exception as e:")
    script_lines.append("    print(json.dumps({'status':'error', 'message': str(e), 'traceback': traceback.format_exc()}, default=str), flush=True)")
    ...
    try:
        ...
        if completed.returncode != 0:
            return {"status": "error", "message": completed.stderr.strip() or "Unknown error", "raw": completed.stdout.strip()}
...
7. **CRITICAL:** Do NOT call `scrape_url_to_dataframe()` if the rules state you have an uploaded dataset.
"""),
    ("human", "{input}"),
...
def run_agent_safely_unified(llm_input: str, pickle_path: str = None) -> Dict:
    try:
        ...
        # Only run scrape tool if no pickle file was provided
        ...
        if exec_result.get("status") != "success":
            return {"error": f"Execution failed: {exec_result.get('message')}", "raw": exec_result.get("raw"), "traceback": exec_result.get("traceback")}
    ...
    try:
            ...
            try:
                if filename.endswith(".csv"):
                    ...
                    if PIL_AVAILABLE:
                        ...
                        df = pd.DataFrame({"image": [image.convert("RGB")]})
                    else:
                ...
                raise HTTPException(400, f"Error processing data file: {str(e)}")

        llm_rules = (
            "Rules:\n"
            f"1) You have access to a pandas DataFrame called `df` and its dictionary form `data`. "
            f"({'DO NOT CALL scrape_url_to_dataframe()' if dataset_uploaded else 'You may use scrape_url_to_dataframe(url) if needed.'})\n"
        ...
        with ThreadPoolExecutor() as ex:
            ...
            try:
                ...
                # Catch a more generic exception to handle any issue from the thread
                raise HTTPException(408, "Processing timeout or unexpected error in agent execution thread.")
        ...
        if keys_list and type_map:
            ...
            for idx, q in enumerate(result.keys()):
                if idx < len(keys_list):
                    ...
                    try:
                        val = result[q]
                        if isinstance(val, str) and (val.startswith("data:image/") or val.startswith("iVBORw")):
...
async def run_in_thread(fn, *a, timeout=30, **kw):
    ...
    except asyncio.TimeoutError: raise TimeoutError("timeout")
...
def _system_info():
    ...
    except Exception: info["cwd_free_gb"] = None
    try:
        info["tmp_free_gb"] = round(shutil.disk_usage(tempfile.gettempdir()).free / 1024**3, 2)
    except Exception: info["tmp_free_gb"] = None
    try:
        import torch
        info["torch_installed"] = True
        info["cuda_available"] = torch.cuda.is_available()
        if torch.cuda.is_available(): info["cuda_device_name"] = torch.cuda.get_device_name(0)
    except Exception:
...
def _installed_packages_sample():
    try:
        ...
        for dist in importlib.metadata.distributions():
            try: out.append(f"{dist.metadata['Name']}=={dist.version}")
...
def _test_gemini_key_model(key, model, ping_text="ping"):
    try:
        from langchain_google_genai import ChatGoogleGenerativeAI
    except Exception as e: return {"ok": False, "error": f"langchain_google_genai import error: {e}"}
    try:
        obj = ChatGoogleGenerativeAI(model=model, temperature=0, google_api_key=key)
        resp = obj.invoke(ping_text, {"timeout": 10})
        text = resp.content if hasattr(resp, 'content') else str(resp)
        return {"ok": True, "model": model, "summary": text[:200] if text else None}
    except Exception as e: return {"ok": False, "error": str(e)}

async def check_network():
    coros = [run_in_thread(_network_probe_sync, url, timeout=30) for url in DIAG_NETWORK_TARGETS.values()]
    results = await asyncio.gather(*coros, return_exceptions=True)
    out = {}
    for (name, _), res in zip(DIAG_NETWORK_TARGETS.items(), results):
        if isinstance(res, Exception): out[name] = {"ok": False, "error": str(res)}
        else: out[name] = res
    return out

async def check_llm_keys_models():
    if not GEMINI_KEYS: return {"warning": "no GEMINI_KEYS configured"}
    results = []
    for model in (MODEL_HIERARCHY or ["gemini-2.5-pro"]):
        tasks = [run_in_thread(_test_gemini_key_model, key, model, timeout=DIAG_LLM_KEY_TIMEOUT) for key in GEMINI_KEYS]
        completed = await asyncio.gather(*tasks, return_exceptions=True)
        model_summary = {"model": model, "attempts": []}
        any_ok = False
        for key, res in zip(GEMINI_KEYS, completed):
            if isinstance(res, Exception): model_summary["attempts"].append({"key_mask": (key[:4] + "..." + key[-4:]) if key else None, "ok": False, "error": str(res)})
            else:
                model_summary["attempts"].append({"key_mask": (key[:4] + "..." + key[-4:]) if key else None, **res})
                if res.get("ok"): any_ok = True
        results.append(model_summary)
        if any_ok: break
    return {"models_tested": results}

@app.get("/summary")
async def diagnose(full: bool = False):
    started = datetime.utcnow()
    report = {"status": "ok", "server_time": _now_iso(), "summary": {}, "checks": {}, "elapsed_seconds": None}
    tasks = {
    ...
    for name, coro in tasks.items():
        ...
        except TimeoutError: results[name] = {"status": "timeout", "error": "check timed out"}
        except Exception as e: results[name] = {"status": "error", "error": str(e), "trace": traceback.format_exc()}
